...
class _EditAssetScreenState extends ConsumerState<EditAssetScreen> {
  ...
  void _loadAsset() {
    final assets = ref.read(syncNotifierProvider).assets;
    final asset = assets.firstWhere((a) => a.id == widget.assetId);
    _originalAsset = asset;

    _selectedOwner = asset.owner;
    _selectedType = asset.assetType;
    _selectedCurrency = asset.currency;
    _institutionController.text = asset.institution;
    _countryController.text = asset.country;
    _amountController.text = asset.amountInCCY.toString();
    _rateController.text = (asset.interestRate * 100).toString();
  }
  ...
  Future<void> _updateAsset() async {
    ...
    await ref.read(syncNotifierProvider.notifier).updateAsset(updatedAsset);
